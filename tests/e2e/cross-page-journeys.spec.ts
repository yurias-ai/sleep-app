...
test('E2E-JOURNEY-003 LINE誘導導線: 黄体期トップのバナー→タップで予告ページ強調へ連携', async ({ page }) => {
  ...
  // 4) #fc-line-id の href がエルメ流入経路URL（アプリ経由タグ付与＋体験入学案内）。
  await expect(page.locator('#fc-line-id')).toHaveAttribute('href', 'https://s.lmes.jp/landing-qr/2006353028-ZkXRAR0K?uLand=HfZHgJ');
});
